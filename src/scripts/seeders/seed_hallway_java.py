...
def load_json(filename):
    ...
    base_path = os.path.join(os.path.dirname(__file__), "..", "data", "java", "hallway")
...
def seed_hallway_java():
    ...
    with Session(engine) as session:
        logging.info("Starting JAV_HALLWAY JSON-based seeding process...")
        ...
        jav_hallway_tag = get_or_create_tag("JAV_HALLWAY")
        ...
        for q_data in tqdm(QUESTIONS, desc="Seeding Questions"):
            q_id = get_uuid(f"jav_hallway_q_{q_data['title']}")
        ...
        for i, r_data in enumerate(tqdm(RIDDLES, desc="Seeding Riddles")):
            r_id = get_uuid(f"jav_hallway_r_{i}")
        ...
        for p_data in tqdm(PROBLEMS, desc="Seeding Problems"):
            ...
            if "test_cases" in p_data:
                ...
                for i, tc_data in enumerate(p_data["test_cases"]):
                    tc_id = get_uuid(f"jav_hallway_tc_{p_data['title']}_{i}")
                    tc = TestCase(
                        id=tc_id,
                        problem_id=p_id,
        ...
        session.commit()
        logging.info("JAV_HALLWAY JSON-based seeding completed successfully.")
